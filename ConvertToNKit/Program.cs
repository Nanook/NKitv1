...
namespace Nanook.NKit.ConvertToNkit
{
    class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            try
            {
                if (args.Length == 0)
                {
                    logLine(string.Format(@"Convert Wii and GameCube image files to NKit
...
", Settings.ExeName));
                ...
                foreach (SourceFile src in files)
                {
                    ...
                    try
                    {
                        ...
                        OutputResults results = nkitConvert.ConvertToNkit(); //apps can use the results
                    }
                    catch (Exception ex)
                    {
